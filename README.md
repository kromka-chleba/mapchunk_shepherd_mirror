...
* `/chunk_add_label <label> [chunk_hash]` - Adds a label to the current mapchunk by default (or to a specified chunk hash), requires `server` privilege
* `/chunk_remove_label <label> [chunk_hash]` - Removes a label from the current mapchunk by default (or from a specified chunk hash), requires `server` privilege
